...
void ON_Model_Ngeq2::makeMeasurement()
{
  ...
  Vector_NDim* mag = getMagnetization();
  double magNormPerSpin = sqrt(mag->getSquare()) / (1.0 * N_);
  ...
  measures.accumulate("mag",           magNormPerSpin);
  measures.accumulate("magSq",         pow(magNormPerSpin, 2));

  delete mag;  // avoid memory leak
}
