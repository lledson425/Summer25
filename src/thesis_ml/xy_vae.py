...
class VAE:
    ...
    def preprocess_images(self, images, rotate=True):
        # Ensure float32 dtype
        images = np.asarray(images, dtype=np.float32)
    
        # Normalize each spin to unit length
        norms = np.linalg.norm(images, axis=-1, keepdims=True)
        images = images / (norms + 1e-8)  # prevent division by zero

        
    
        # Optional: random rotation for entire lattice
        if rotate:
            angle = np.random.uniform(0, 2 * np.pi)
            rot_matrix = np.array([[np.cos(angle), -np.sin(angle)],
                                   [np.sin(angle),  np.cos(angle)]], dtype=np.float32)
            images = np.einsum('...ij,jk->...ik', images, rot_matrix)

        images = (images + 1.0) / 2.0

        if np.isnan(images).any():
            raise ValueError("Input contains NaNs after normalization.")
    ...
    class CVAE(tf.keras.Model):
        def __init__(self, latent_dim, image_dim, **kwargs):
            ...
            self.encoder = tf.keras.Sequential(
                [tf.keras.layers.InputLayer(input_shape=(image_dim, image_dim, 2))] +
                conv_layers +
            ...
            self.decoder = tf.keras.Sequential(
                ...
                deconv_layers +
                [tf.keras.layers.Conv2DTranspose(2, 4, strides=1, padding='same')]
            )
